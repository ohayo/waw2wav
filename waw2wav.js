...
function processWavFile(inputFile, outputFile, lookupTable) {
...
if (args.length < 2) {
  console.log("---- WAW2WAV USAGE -----");
  console.log("waw2wav.js <PATH_TO_WAW_FILE> <GAME_CODE>");
  ...
  console.log("------------------------");
  console.log("Output files will be the same path as <PATH_TO_WAW_FILE> but with .wav on the end instead.");
  return;
}

const wawPath = args[0];

if (!wawPath.endsWith(".waw")) {
    throw new Error('There is no ".waw" extension on this file. Are you sure this is the right file?');
}
...
processWavFile(wawPath, path.basename(wawPath).split('.')[0] + ".wav", keyBinPath)
